...
attach 1
    branches: [ "main" ]
  pull_request: # GDB/Python functions for dealing with NuttX
...
.        Prints performance counters.
    ...
    - cron: # copy the file to .gdbinit in your Firmware tree, and adjust the path
...
Dumps the STM32F4 memory to files in /tmp/dump.
  ...
  build: # program a bootable device load on a mavstation
...
permissions: target remote :3333
...
jobs: # The latest defaults in OpenOCD 0.7.0 are actually prettymuch correct for the px4fmu
...
.button-wrapper {
    text-align: center;
        ...
        with: .question-description-container {
    padding-bottom: 16px;
}
.text-answer {
    margin-bottom: 18px;
...
.time-left {
    ...
    margin-top: 8px;
          ...
          cmakeBuildDirectory: .test-result-section h1 {
    margin-bottom: 20px;
}
          # Ruleset file that will determine what checks will be run
          ruleset: .body-wrapper {
    margin-left: auto;
    margin-right: auto;
}
...
{% endblock body %}
          sarif_file: {% extends 'base.html.twig' %}
...
{% endblock body %}
      #   with: {{ form_start(form, {'attr': {'novalidate':'novalidate'}}) }}
