...
}  # for checkmarx-ts/checkmarx-cxflow-github-action to write feedback to github issues
      pull-requests: # This lets you log in via `eb ssh` and access the docker daemon.
...
commands:
  0_add_docker_group_to_ec2_user:
    ...
    test: groups ec2-user | grep -qv docker  # for checkmarx-ts/checkmarx-cxflow-github-action to write feedback to PR
      security-events: {"__symbol_cache_version__":11}  # for github/codeql-action/upload-sarif to upload SARIF results
      actions: <?php

phutil_register_library('customlib', __FILE__);  # only required for a private repository by github/codeql-action/upload-sarif to get the Action run status
    runs-on: <?php
