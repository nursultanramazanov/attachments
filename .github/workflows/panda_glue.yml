...
PROJECT_NAME           = "Px4 Hardware-In-the-Loop(HIL) User Guide for Qualcomm Eagle"
...
USE_MDFILE_AS_MAINPAGE = ./UserGuide.md
...
PROJECT_NAME           = "PX4 Autopilot Firmware"
...
# Simple version: FILE_VERSION_FILTER = "/bin/sh -c 'git log --pretty=\"format:%ci\" -1 \"${1}\" || echo no git'"
FILE_VERSION_FILTER = "sh versionfilter.sh"
...
DOCSET_BUNDLE_ID       = ch.ethz.pixhawk.px4
...
git log --pretty=format:"Last change: commit %h - %aN, %ar : %s" -1 $1 || echo no git
  ...
  - title: <strong>[news]</strong>
    post_list:
      limit: 3
    runs-on: <a href="{{ "/" | relative_url }}">{{ site.theme_config.back_home_text }}</a>
  ...
  <link rel="stylesheet" href="{{ "/assets/css/main.css" | relative_url }}" />
      ...
      - name: {%-for item in include.collection-%}
  {%- if item.url -%}
...
<article>
  ...
  {{ content }}
        ...
        id: // Forked from MIT-licensed https://github.com/riggraz/no-style-please; see LICENSE
...
body {
        ...
        font-size: 0.875rem; // default: 16px
...
#toc li > p {
        margin: 0;
          ...
          cmakeBuildDirectory: ${{ /* document.addEventListener("click", (e) => { 
    var id = e.target.id; 
    if (id) openMenu(id); 
}, true); */
function openMenu(t) {
...
fn main() {
    ...
    for proto in PROTOS {
      ...
      #     name: // Copyright 2023 Signal Messenger, LLC
// SPDX-License-Identifier: AGPL-3.0-only
...
module.exports = {
  ...
  create(context) {
    return {
      ...
      #     path: ${{ // Copyright 2023 Signal Messenger, LLC
// SPDX-License-Identifier: AGPL-3.0-only
